...
namespace project
{
    public partial class hosit : Form
    {
        public List<string> SelectedSeats = new List<string>();


        public hosit(string selectedDate, string selectedTime)
        {
            InitializeComponent();
            label2.Text = selectedDate + " " + selectedTime + " 좌석을 선택하시오";


            this.Load += hosit_Load;
        }


        private void hosit_Load(object sender, EventArgs e)
        {
            int rowCount = 5;
            int colCount = 10;
            int startX = 50;
